...
- Centralized services in `VLAN 40`, including file, admin (w/DNS, NTP), web, and DHCP servers
...
| Admin Server | `admin.services.local` | `192.168.40.2` | DNS and NTP service for the lab |
...
15. Apply ACL policy controls and validate source-based filtering behavior.
16. Test failover, path selection, name resolution, and end-to-end connectivity.
...
- Core shared services are centralized in **VLAN 40**, with static server IPs, DHCP scopes for the user VLANs, and the admin server providing both DNS and NTP.
